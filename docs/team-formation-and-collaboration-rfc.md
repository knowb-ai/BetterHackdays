...
- an optional prefix or namespace for room and team keywords
...
## Slugs and Room/Team Keywords

Participants should be able to exchange simple slugs or suffix keywords instead
of many different identifiers.

The system should not require every room or team slug to start with an event
prefix. If a slug is temporary, unique, and resolvable on its own, participants
should be able to use it directly.

The slug can be:

- generated by the system
- chosen by the user or team
- created before an event association exists
- connected to an official event later
- connected to event context after the room or team already exists

This means a team could create a memorable slug first and later attach it to an
official event or event channel. Once attached, the slug can inherit the
overarching event context, such as rules, timeline, sponsors, judging criteria,
allowed tools, and submission requirements.

Prefixes can still be useful for disambiguation, official event discovery, or
large events with many rooms. They should be a resolution option, not the only
valid format.

The slugs and keywords should be:
...
codes. A simple English word list may be useful in some cases, but it should
not be the only option. Hackathons can happen in different languages and
cultural contexts, where an English word may be hard to remember, pronounce, or
spell.
...
- allow direct standalone slugs when they are globally unique
...
- support language-aware or event-local word lists
- let event organizers attach existing slugs to official event context
...
4. BetterHackdays resolves the temporary slug or team keyword.
...
pre-authentication, custom slugs, and room creator configuration.
...
- standalone room or team slug
- optional event-to-slug attachment
...
- event namespaces may be needed for disambiguation
- standalone slugs need collision, ownership, and reassignment rules
- long-lived rooms may need different handling than one-day rooms

## Open Questions

Event, room, and slug resolution:

- What exact format should event codes and standalone slugs use?
- Should codes be URL paths, prompt tokens, QR payloads, or all of them?
- Should team slugs be one word, two words, custom, or event-generated?
- Should codes be globally unique or only unique within an event?
- Can standalone slugs be used without an event prefix?
- When should an event prefix be required for disambiguation?
- Can a team create a slug first and attach it to an official event later?
- Who is allowed to attach or detach a slug from event context?
- What happens if a user-chosen slug conflicts with an official event slug?
- How should multilingual events choose memorable, pronounceable slugs?
